...
onde $\tau_W$ é o imposto sobre riqueza. Chamando a riqueza no fim do investimento de $W_{1}$, temos que a CPO é: $$\mathbb{E} \left\{ u^{\prime}(W_1) (1 - \tau_W) (x - r) \right\} = 0 = \mathbb{E} \left\{ u^{\prime}(W_1)(x - r) \right\}$$
...
Isso não vale apenas para ativos financeiros: um exemplo de investimento arriscado é a **educação**, já que ela implica gastos (pelo menos em custos de oportunidade) presentes para retornos incertos e bastante variáveis no futuro

Se o gasto em educação é de graça ou dedutível, então há **compensação por perdas** no investimento &mdash; ademais, como imposto de renda só é cobrado sobre renda recebida, os custos de oportunidade também são dedutíveis

Então tributação de renda *aumentaria* o investimento em educação &mdash; mas na prática o imposto de renda é progressivo, o que torna a *compensação de perdas não completa*, e portanto **o efeito é ambíguo**
...
`r Citep(myBib, "agersnap2021tax")` estimam a elasticidade da receita em relação à taxa sobre ganhos de capital em entre -0,5 e -0,3, negando o argumento comum que cortes no imposto "se pagariam": a taxação que maximiza receita é entre 38-47%

`r Citep(myBib, "goupille2018behavioral")` e `r Citep(myBib, "glogowsky2021behavioral")` estimam elasticidades da riqueza tributável em relação a taxação de heranças baixas: 0,27 e 0,1, respectivamente
...
Similarmente, nos Países Baixos uma redução de 1p.p. aumentou a riqueza reportada em 14% &mdash; mas pesquisa para a Suécia acha um valor muito menor de 0.1-0.27%
...
Taxação de riqueza também pode incentivar emigração: a evidência é que esse efeito é significativo, mas nela o imposto é regional (e na Espanha o efeito veio inteiramente de declaração fraudulenta)
