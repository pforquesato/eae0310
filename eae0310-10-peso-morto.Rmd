...
Seja $e \left( p_0, V(p_1, Y) \right)$ a renda necessária para alcançar a preços iniciais $p_0$ a mesma utilidade que o indivíduo tem a preços após os impostos $p_1$ (utilidade que é menor, já que $p_1 > p_0$)
...
É natural que se alguém der `$`1 para o governo, ele terá uma perda de bem-estar de (pelo menos) `$`1 &mdash; mas na verdade será maior que isso, pois transferir renda entre agentes econômicos é custoso (*leaky bucket*)
...
**O que gera peso morto é o efeito substituição** &mdash; *efeito renda* não gera peso-morto

De fato, *impostos lump-sum* também geram efeito renda &mdash; sem alteração nos preços relativos, a variação equivalente é apenas a transferência de recursos (peso morto se mede pela **demanda compensada**)

O peso morto vem da *alteração dos preços relativos*: transferências universais não alteram o preço relativo (p. ex. entre consumo e lazer) &mdash; efeito renda move a curva de demanda, não gera um **tax wedge**
