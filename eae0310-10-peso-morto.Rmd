...
A função de demanda compensada sempre é negativamente inclinada, pois ao manter o consumidor na mesma curva de indiferença, elimina o efeito renda (ambíguo) e mantém apenas o efeito subsituição (sempre negativo) (Nicholson & Snyder) 

---
class: middle

```{r, echo=FALSE, out.width = '75%', fig.align = 'center'}
knitr::include_graphics("figs/eae0310-10-7.png")
```

Imagine que um imposto $t > 0$ mova $p^{\prime \prime}$ para $p^{\prime} = p^{\prime \prime} + t$, isso reduz a demanda compensada de $x^{\prime \prime}$ para $x^*$, a quantidade ótima na curva de indiferença $U_2$ &mdash; a preços relativos $p^{\prime}/p_y$ o agente precisaria de mais renda para consumir $x^{\prime \prime}$: essa é a **variação equivalente** (N&S) 
...
No ótimo, o imposto deve manter a taxa marginal de substituição técnica entre insumos igual em todos os seus usos produtivos: qualquer imposto sobre a produção distorce a produção **e** o consumo, então deve ser pior que um que distorce apenas o consumo
...
Mas nas últimas décadas se tornou muito mais comum o **imposto sobre valor adicionado** (IVA, ou VAT em inglês): com *compliance* perfeito e sem isenções, o IVA é equivalente a um imposto sobre vendas
...
Vimos ainda nessa aula que $\text{EB}_k = \frac{1}{2}\frac{\epsilon_S^k \epsilon_D^k}{\epsilon_S^k - \epsilon_D^k} \frac{Q_k}{p_k} (t_k)^2$. Portanto: $$\min_{(t_k)_{k=1}^K} \sum_{k=1}^K \frac{1}{2}\frac{\epsilon_S^k \epsilon_D^k}{\epsilon_S^k - \epsilon_D^k} \frac{Q_k}{p_k} (t_k)^2 + \lambda \cdot \left( \sum_{k=1}^K Q_k t_k - \bar{R} \right)$$
...
$$[t_k]: \frac{\epsilon_S^k \epsilon_D^k}{\epsilon_S^k - \epsilon_D^k} \frac{Q_k}{p_k} t_k = - \lambda Q_k \Rightarrow \frac{t_k}{p_k} = \lambda \left( \frac{1}{|\epsilon_D|} + \frac{1}{|\epsilon_S|} \right)$$
...
Outro problema é que assume que podemos olhar cada mercado separadamente, o que sabemos que não é verdade: há efeitos importantes de *equilíbrio geral* (elasticidades cruzadas)
...
O **princípio da focalização** (Atkinson & Stiglitz, 1976) diz que se podemos taxar a renda diretamente, devemos usar esse instrumento para reduzir desigualdade de renda, não a taxação de consumo (ou de poupança)

Mas isso só vale se a capacidade de geração de renda for a única fonte de desigualdade. Se houver outras dimensões (por exemplo, riqueza ou desconto intertemporal), isso já não é mais válido
